...
from modeling.imagebind.models.helpers import (
    EinOpsRearrange,
...
from modeling.imagebind.models.multimodal_preprocessors import (
    AudioPreprocessor,
    IMUPreprocessor,
    PadIm2Video,
    PatchEmbedGeneric,
...
from modeling.imagebind.models.transformer import (
    MultiheadAttention,
    SimpleTransformer,
)
...
def imagebind_huge(pretrained=True, path=None):
    model = ImageBindModel(
        vision_embed_dim=int(os.getenv('VISION_EMBED_DIM', '1280')),
        vision_num_blocks=int(os.getenv('VISION_NUM_BLOCKS', '32')),
        vision_num_heads=int(os.getenv('VISION_NUM_HEADS', '16')),
        text_embed_dim=int(os.getenv('TEXT_EMBED_DIM', '1024')),
        text_num_blocks=int(os.getenv('TEXT_NUM_BLOCKS', '24')),
        text_num_heads=int(os.getenv('TEXT_NUM_HEADS', '16')),
        out_embed_dim=int(os.getenv('OUT_EMBED_DIM', '1024')),
        audio_drop_path=float(os.getenv('AUDIO_DROP_PATH', '0.1')),
        imu_drop_path=float(os.getenv('IMU_DROP_PATH', '0.7')),
    )

    if pretrained:
        print(f'Pretrained model from path {path}')
        if path is None:
            path = '/mnt/chatbot_models2/e.rykov/team_code/ckpts/imagebind/imagebing_huge.pth'
